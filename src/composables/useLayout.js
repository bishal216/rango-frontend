...
const STORAGE_KEY = "appThemeConfig";
...
function loadThemeConfig() {
  try {
    ...
    if (saved) {
      ...
      if (parsed.primary) appState.value.primary = parsed.primary;
      if (parsed.surface !== undefined) appState.value.surface = parsed.surface;
      if (typeof parsed.darkMode === "boolean")
        appState.value.darkMode = parsed.darkMode;
    }
  } catch (e) {
...
function setPrimary(value) {
  ...
  const color = primaryColors.value.find((c) => c.name === value);
...
function setSurface(value) {
  ...
  const surfaceColor = surfaces.value.find((s) => s.name === value);
...
// Immediately apply palettes on load
...
export function loadAndApplyThemeConfig() {
  loadThemeConfig();

  // If no surface, set default
  if (!appState.value.surface) {
    appState.value.surface = appState.value.darkMode ? "zinc" : "slate";
  }

  // Immediately update palettes synchronously
  if (appState.value.primary) {
    const color = primaryColors.value.find(
      (c) => c.name === appState.value.primary
    );
